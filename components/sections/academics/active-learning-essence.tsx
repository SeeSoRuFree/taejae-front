...
interface ActiveLearningEssenceProps {
  locale: Locale
}

export function ActiveLearningEssence({ locale }: ActiveLearningEssenceProps) {
  const { t } = useTranslation(locale)

  const description = t('academics.howLearn.essence.description') as string[]
          ...
          <div className="flex flex-col gap-11 w-full">
            <h2 className="font-eb-garamond font-normal leading-[1.1] max-w-[960px] text-[64px] text-black tracking-[-1.28px] mobile:text-[40px] mobile:tracking-[-0.8px]">
              <span className="text-black">
                {t('academics.howLearn.essence.titlePrefix')}
              </span>
              <span className="text-[#1da597]">
                {t('academics.howLearn.essence.titleHighlight')}
              </span>
            </h2>
          </div>
        </div>
            ...
            <div className="font-rethink-sans font-normal leading-[1.5] text-[#111111] text-[20px] tracking-[-0.6px] w-full mobile:text-[18px] mobile:tracking-[-0.54px]">
              {description.map((paragraph, index) => (
                <div key={index}>
                  ...
                  {index < description.length - 1 && <p className="mb-0">&nbsp;</p>}
                </div>
              ))}
            </div>
          </div>
